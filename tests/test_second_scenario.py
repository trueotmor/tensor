...
def test_second_scenario():
    ...
    try:
        ...
        # Проверка перехода на страницу контактов
        ...
        for partner in partners_info[:3]:  # Показываем первые 3 партнера
            logger.info(f"Партнер {partner['number']}: {partner['name']} - {partner.get('address', 'адрес не указан')}")
        
        # Проверяем что есть хотя бы один партнер
        ...
        driver.save_screenshot("error_region_scenario.png")
        logger.info("📸 Скриншот ошибки сохранен: error_region_scenario.png")
        raise
    finally:
        driver.quit()
